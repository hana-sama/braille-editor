...
export class UEBGrade1Mode extends BrailleMode {
  private alphabet: Record<number, string> = {};
  private numberMap: Record<number, string> = {};
  private symbols: Record<number, string> = {};
  private contextDependent: Record<number, ContextDependentEntry> = {};
  private indicatorCodes: IndicatorCodes;
  ...
  private _buildPrefixLookup(): void {
  ...
  codeToText(code: BrailleCode, context: EditorContext = {}): string {
    const state = context.state ?? ({} as Partial<ModeState>);
    ...
    if (this.alphabet[code]) {
      ...
      if (state.capitalMode === 1 || state.capitalMode === 2) {
  ...
  getAlphabet(): Record<number, string> {
  ...
  private _resolveContextDependent(
    code: BrailleCode,
    ...
    if (entry.rule === "space_or_start_before") {
      const lastChar = context.precedingText
        ? context.precedingText.slice(-1)
        : "";
  ...
  resolveSequence(
    prefixCode: BrailleCode,
    baseCode: BrailleCode,
  ...
  updateState(state: ModeState, code: BrailleCode, text: string): ModeState {
  ...
  applyIndicator(state: ModeState, indicatorName: string): ModeState {
    const indicator = this.indicators[indicatorName];
  ...
  isLetterAtoJ(code: BrailleCode): boolean {
    return code in this.numberMap;
  }

  /** Check if code is a letter a-z. */
  isLetter(code: BrailleCode): boolean {
    return (
      this.alphabet[code] !== undefined && this.alphabet[code].length === 1
    );
  }

  /** Recalculate number mode from braille content. */
  recalculateNumberMode(brailleContent: string): boolean {
